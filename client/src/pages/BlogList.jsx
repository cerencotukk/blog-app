          ...
          {posts.map((post) => (
            <div key={post.id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 group">
              <div className="p-6">
                ...
                <Link
                  to={`/blog/${post.id}`}
                  className="mt-4 inline-block w-full text-center bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-4 py-2 rounded-md 
                    hover:opacity-90 transition-all shadow-sm hover:shadow group-hover:scale-[1.03] duration-300 font-semibold"
                >
                  Devamını Oku
                  <svg className="w-4 h-4 ml-1 inline-block transform translate-y-0 transition-transform duration-300 group-hover:translate-x-1" 
                    fill="none" 
                    stroke="currentColor" 
                    viewBox="0 0 24 24"
                  >
